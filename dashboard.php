...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <span class="badge bg-info">Live <span class="blink" style="display:inline-block; width:8px; height:8px; background:white; border-radius:50%; animation:blink 1s infinite;"></span></span>
    </div>
    <div class="card-body">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($recent_scans as $scan): ?>
                            <tr>
                                <td><strong><?php echo htmlspecialchars($scan['first_name'] . ' ' . $scan['last_name']); ?></strong></td>
                                <td>
                                    <?php
        ...
        fetch('api/get_recent_scans.php')
            ...
            .then(data => {
                ...
                if (data.scans.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="4" class="text-center text-muted py-4">No scans today yet</td></tr>';
                } else {
                    data.scans.forEach(scan => {
                        const statusClass = scan.status === 'present' ? 'bg-success' : scan.status === 'late' ? 'bg-warning' : 'bg-danger';
                        const statusText = scan.status === 'present' ? '✓ Present' : scan.status === 'late' ? '⚠ Late' : '✗ Absent';
                        const session = scan.time_in_am ? 'AM' : 'PM';
                        const time_obj = scan.time_in_am ? scan.time_in_am : scan.time_in_pm;
                        const time = new Date('1970-01-01T' + time_obj).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
